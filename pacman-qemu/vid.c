...
int kpchar(unsigned char c, int ro, int co)
{
   int x, y;
   x = co*8;
   y = ro*16;
...
int unkpchar(unsigned char c, int ro, int co)
{
   int x, y;
   x = co*8;
   y = ro*16;
...
int putback(int buf[16][16],int oldstartRow, int oldstartCol){
  ...
  for (y=0,i=(oldstartRow * 16); i<h+(oldstartRow * 16); i++,y++){
    for (x=0,j=(oldstartCol * 16); j<(oldstartCol * 16)+w; j++,x++){
      fb[i*640 + j] = buf[y][x];
    }
  }
}
...
{ 
 ...
 int yy=0,xx=0;
   ...
   // p += h*r2;
   p += (h-1)*r2;
   for (xx=0,i=(startRow * 16); i<h+(startRow * 16); i++,xx++){   //swapped x and y as my logic for walls was flipped
     pp = p;
     // for (j=startCol; j<w+startCol; j++){
     for (yy=0,j=startCol * 16; j<(startCol * 16)+w; j++,yy++){
         b = *pp; g = *(pp+1); r = *(pp+2);
         pixel = (b<<16) + (g<<8) + r;
...
{ 
  int h, w, pixel, r1, r2, i, j; 
   ...
   // p += h*r2;
   p += (h-1)*r2;
   for (i=startRow * 16; i<h+(startRow * 16); i++){
     pp = p;
     // for (j=startCol; j<w+startCol; j++){
     for (j=startCol * 16; j<(startCol * 16)+w; j++){
         b = *pp; g = *(pp+1); r = *(pp+2);
         pixel = (b<<16) + (g<<8) + r;
         fb[i*640 + j] = pixel;
         pp += 3;    // back pp by 3 bytes
